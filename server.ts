...
app.use('/dist', (req: Request, res: Response, next: NextFunction): void => {
    const originalUrl: string = req.url;
    
    // Si la URL no tiene extensión, agregar .js
    if (!path.extname(originalUrl) && !originalUrl.endsWith('/')) {
        const jsPath: string = originalUrl + '.js';
        const fullPath: string = path.join(PROJECT_ROOT, 'public/dist', jsPath);
...
app.get('/favicon.ico', (req: Request, res: Response): void => {
    const faviconSVG: string = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100">
        <defs>
...
app.post('/api/save-file', (req: Request<{}, ApiResponse, SaveFileRequest>, res: Response<ApiResponse>): void => {
    try {
        const { filename, content }: SaveFileRequest = req.body;
        ...
        const filePath: string = path.join(PROJECT_ROOT, 'uploads', filename);
        
        // Crear directorio uploads si no existe
        const uploadsDir: string = path.join(PROJECT_ROOT, 'uploads');
...
app.get('/api/load-file/:filename', (req: Request<{ filename: string }>, res: Response<ApiResponse>): void => {
    try {
        const { filename }: { filename: string } = req.params;
        const filePath: string = path.join(PROJECT_ROOT, 'uploads', filename);
...
app.use((req: ErrorRequest, res: Response, next: NextFunction): void => {
...
const checkDirectories = (): void => {
    ...
    requiredDirs.forEach((dir: string): void => {
...
const checkCompiledFiles = (): boolean => {
    const distDir: string = path.join(PROJECT_ROOT, 'public/dist');
    ...
    const missingFiles: string[] = requiredFiles.filter((file: string): boolean => 
        !fs.existsSync(path.join(distDir, file))
    );
    
    if (missingFiles.length > 0) {
        console.log('⚠️  Archivos compilados faltantes:');
        missingFiles.forEach((file: string): void => console.log(`   - public/dist/${file}`));
    ...
    if (fs.existsSync(distDir)) {
        ...
        const walkDir = (dir: string, prefix: string = ''): void => {
            ...
            files.forEach((file: string): void => {
                ...
                const relativePath: string = path.relative(distDir, fullPath);
...
const startServer = (): void => {
    ...
    const compiledFilesExist: boolean = checkCompiledFiles();
    listCompiledFiles();
