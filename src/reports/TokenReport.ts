...
<body>
    ...
    private static getTokenClass(type: string): string {
        if (type.includes('Reservada')) return 'reservada';
        if (type.includes('Texto')) return 'texto';
        if (type.includes('Número')) return 'numero-token';
        return 'simbolo';
    }
}
