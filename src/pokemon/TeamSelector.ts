...
export class TeamSelector {
    ...
    private parseStats(): { health: number; attack: number; defense: number } | null {
    ...
    private isTypeToken(tokenType: string): boolean {
        return [
            ...
            TokenType.DRAGON,
            TokenType.BICHO
        ].includes(tokenType as TokenType);
    }

    private isStatToken(tokenType: string): boolean {
        return [
            TokenType.SALUD,
            TokenType.ATAQUE,
            TokenType.DEFENSA
    ...
    private check(tokenType: string): boolean {
